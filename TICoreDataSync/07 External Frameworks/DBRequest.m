...
- (id)initWithURLRequest:(NSURLRequest*)aRequest andInformTarget:(id)aTarget selector:(SEL)aSelector {
    NSLog(@"%s %@", __PRETTY_FUNCTION__, [aRequest URL]);
    if ((self = [super init])) {
...
- (void)setError:(NSError *)theError {
	...
	if (!([error.domain isEqual:DBErrorDomain] && error.code == 304)) {
		...
		DBLogWarning(@"DropboxSDK: error making request to %@ - (%d) %@", [request URL], error.code, errorStr);
        DBLogWarning(@"Request header fields: %@", [request allHTTPHeaderFields]);
        DBLogWarning(@"Response header fields: %@", [response allHeaderFields]);
	}
}
