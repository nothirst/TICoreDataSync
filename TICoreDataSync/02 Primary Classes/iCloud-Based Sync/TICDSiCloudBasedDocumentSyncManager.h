//
//  TICDSiCloudBasedDocumentSyncManager.h
//  ShoppingListMac
//
...
/** The `TICDSiCloudBasedDocumentSyncManager` describes a class used to synchronize documents with a remote service that can be accessed via an `NSFileManager`. This includes:
 
 1. Dropbox on the desktop (files are typically accessed via `~/Dropbox`)
 2. iDisk on the desktop
 
 No iCloudBased-specific settings are required when you create a `TICDSiCloudBasedDocumentSyncManager`--the `applicationDirectoryPath` is set automatically when you register (based on the properties set on the `TICDSiCloudBasedApplicationSyncManager`).
 ...
 */
@interface TICDSiCloudBasedDocumentSyncManager : TICDSDocumentSyncManager {
@private
...
/** Configures the iCloud-based document sync manager to watch for changes uploaded by other clients, and initiate a sync operation automatically when new changes are detected. */
...
/** The path to the root of the application. This will be set automatically when you register and supply a `TICDSiCloudBasedApplicationSyncManager`. */
